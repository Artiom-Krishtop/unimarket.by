...
if (!function_exists("PrintPropsForm"))
{
	function PrintPropsForm($arSource = array(), $locationTemplate = ".default")
	{
		...
		{
			?>
			<div class="flex_item_data_order">
				<div class="title_block_rekv"><h4>Реквизиты</h4></div>
				<?

				foreach ($arSource as $arProperties)
				{
					?>
					<? if ($arProperties["TYPE"] == "RADIO"){ echo '<div style="width: 100%;"></div>'; } ?>
					<div data-property-id-row="<?=intval(intval($arProperties["ID"]))?>">
						...
						elseif ($arProperties["TYPE"] == "TEXT")
						{
							?>
							<div class="r3x1">
								<input type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder='<?=$arProperties["NAME"]; if($arProperties["REQUIED_FORMATED"]=="Y"){ echo " *"; }?>'/>
								<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
									<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
								<?endif?>
							</div>
						...
						elseif ($arProperties["TYPE"] == "TEXTAREA")
						{
							...
							?>
							<div class="r3x1">
								<textarea rows="<?=$rows?>" cols="<?=$arProperties["SIZE1"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>"  placeholder='<?=$arProperties["NAME"]; if($arProperties["REQUIED_FORMATED"]=="Y"){ echo " *"; }?>'><?=$arProperties["VALUE"]?></textarea>
								<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
									<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
								<?endif?>
							</div>
						...
						{
							?>
							<div class="r3x1 location_block_order">
								<?
								$value = 0;
						...
						{
							?>
							<div class="title_item_data_order">Желаемый способ связи</div>
							<div class="bx_block r3x1">
								<?
						...
						{
							?>
							<div class="bx_block file_block_order r3x1">
								<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
									<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
								<?endif?>
								<div class="for_file">
									<?=showFilePropertyField("ORDER_PROP_".$arProperties["ID"], $arProperties, $arProperties["VALUE"], $arProperties["SIZE1"])?>
									<div class="a_file_order">Прикрепить</div>
								</div>

								<div class="for_load_order_file">
									<div class="line_load">
										<div class="result_line_load"><div class="ok_ld"></div></div>
										<div class="remove_file_order"><img src="/bitrix/templates/eshop_bootstrap_green/images/closeu.png"></div>
									</div>
									<div class="info_file_order"></div>
								</div>
							</div>
				...
				?>
				<div class="dop_info_order">
					<div class="title_item_data_order">Дополнительная инормация</div>
					<textarea name="ORDER_DESCRIPTION" id="ORDER_DESCRIPTION" placeholder="Дополнительная информация..."><?=$arResult["USER_VALS"]["ORDER_DESCRIPTION"]?></textarea>
