...
class UpdateLock
{
	public static function save($entity_id, $entity_type, $object) {
		...
		\SProdIntegration::Log('(UpdateLock::save) ' . $entity_type . ' id ' . $entity_id . ' save hash');
		return true;
	}
	...
	public static function isChanged($entity_id, $entity_type, $object, $update=false) {
		...
		\SProdIntegration::Log('(UpdateLock::isChanged) ' . $entity_type . ' id ' . $entity_id . ' '.($res?'changed':'no changes'));
		return $res;
	}
